...
const char* cmNN::getActivationFunctionName(double w,
                                            double minW,
                                            double maxW) {
  double rangeI = maxW - minW;
  double out = (w - minW) / rangeI;
  int fxIndex = (int)(out * static_cast<int>(COUNT));
  return getActivationFunctionName(static_cast<NN_ACTIVATION_FX>(fxIndex));
}

int cmNN::getActivationFunctionIndex(double w, double minW, double maxW) {
  double rangeI = maxW - minW;
  double out = (w - minW) / rangeI;
  int fxIndex = (int)(out * static_cast<int>(COUNT));
  return fxIndex;
...
double cmNN::fxUndefined(double input) {
  // printf(
  // "[WARNING]: Activation function not defined, using Identity instead.\n");
...
double cmNN::fxGELU(double input) {
  return (0.5 * input) *
         (1.0 + tanh(sqrt(2.0 / M_PI) * (input + (0.044715 * pow(input, 3)))));
}
