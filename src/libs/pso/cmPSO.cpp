...
void Particle::evolve() {
  ...
  for (size_t d = 0; d < _dimension; d += 1) {
    ...
    _velocity[d] =
        (_iW * _velocity[d]) +
        ((_cW * unif(gen)) * (_bestPosition[d]) - _position[d]) +
        ((_sW * unif(gen)) * (_bestGlobalPosition[d]) - _position[d]);

    _velocity[d] = _position[d] > _maxP || _position[d] < _minP
                       ? _velocity[d] * -1.0
                       : _velocity[d];
...
void Swarm::updateBest() {
  ...
  if (update) {
    // printf("\t\t\t\t Best: %.15lf",
    // _particle[_bestParticle].getBestFitness());
...
void Swarm::compute(size_t maxThreads) {
  ...
  while (thrCreated < _population) {
    ...
    for (size_t ct = 0; ct < thrPerBatch; ct += 1) {
      ...
      int rc = pthread_create(&threads[created], &attr, &Swarm::evaluateThread,
                              (void*)&args[created]);
      if (rc == 0) {
        thrCreated += 1;
        created += 1;
    ...
    thrPerBatch = thrCreated + thrPerBatch > _population
                      ? _population - thrCreated
                      : thrPerBatch;
    delete[] threads;
    delete[] args;
  }
